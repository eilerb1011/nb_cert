...
if __name__ == "__main__":
    ...
    if FORCE_REGISTER or not account:
        ...
        record = (account["fulldomain"])
        # Run the Bash script to update DNS with the CNAME record
        bash_command = f"""
        DOMAIN="{DOMAIN}"
        LINDOMAIN="${{DOMAIN#*.}}"
        ...
        record="{record}"
        TOKEN="{TOKEN}"
        LINDNS_ID=$(curl --request GET https://api.linode.com/v4/domains -H 'accept: application/json' -H "authorization: Bearer $TOKEN" -H 'content-type: application/json' | jq -r --arg domain "$LINDOMAIN" '.data[] | select (.domain == $domain) | .id ')
        curl --request POST --url https://api.linode.com/v4/domains/$LINDNS_ID/records -H "accept: application/json" -H "content-type: application/json" -H "authorization: Bearer $TOKEN" --data '
        {{
          "type": "CNAME",
